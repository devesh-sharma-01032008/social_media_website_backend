...
class MongoController:
    ...
    def get_all_collection_starts_with(self,identifier,separator="-"):
        self.all_collections = self.db.list_collection_names()
        result = []
        for collection in self.all_collections:
            if collection.split(separator)[0] == identifier:
                result.append(collection)
        return result
    def get_some_posts(self,hide_fields=["_id","password","name"]):
        result = self.get_all_collection_starts_with("posts","-")
        if self.current_user >= len(result):
            self.current_user = 0
        self.change_collection(result[self.current_user])
        self.set_collection()
        posts = []
        self.current_user += 1
        for x in self.collection.find():
            ...
            for key in x:
                if key in hide_fields:
                    ...
                    required[key] = x[key]
            posts.append(required)
        posts_not_empty = []
        for i in posts:
            if i == {}:
                continue
            posts_not_empty.append(i)
        return posts_not_empty
